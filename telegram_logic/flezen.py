...
from flezen.public_api import get_flezen_info, extract_flezen_id, FlezenError

log = logging.getLogger(__name__)

# Flezen shares its own cache bucket / user mode.
FZ_MODE = "fz"
...
#! ONLY PUBLIC API
async def process_flezen(event, flezen_url: str) -> None:
    # If currently in flood cooldown → queue immediately
    rem = terabox_queue.flood_remaining(event.chat_id)
    if rem > 0:
        await terabox_queue.put(_fz_helper, event, flezen_url)
        try:
            await event.respond(
    ...
    async with terabox_queue.semaphore:
        try:
            await _fz_helper(event, flezen_url)
        except FloodWaitError as e:
...
async def _fz_helper(event, flezen_url: str) -> None:
    """Inner pipeline, runs under the concurrency semaphore."""
    chat_id = event.chat_id
    link_id = extract_flezen_id(flezen_url) or flezen_url
    cache_source_url = (
        f"https://flezen.com/s/{link_id}"
        if extract_flezen_id(flezen_url)
        else flezen_url
    )
    user_mode = FZ_MODE
    ...
    if cached:
        ...
        try:
            ...
            log.error(f"Flezen metadata fetch failed for {link_id}: {e}")
